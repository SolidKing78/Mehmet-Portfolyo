    ...
    >
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        <Reveal>
          ...
          <h2 className="font-display mt-3 text-xl font-semibold text-[var(--color-fg)] sm:text-2xl">
            Özlü kayıtlar
          </h2>
        </Reveal>
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          {credentials.map((c, i) => (
            <Reveal key={c.label} delay={0.05 * i}>
              <div className="flex h-full flex-col rounded-xl border border-[var(--color-border)] bg-[var(--color-canvas)] p-5">
                ...
                {c.href ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-4 inline-flex text-xs font-semibold text-[var(--color-accent)] hover:underline"
                  >
                    {c.hrefLabel ?? "İndir / aç"}
                  </a>
                ) : null}
              </div>
            </Reveal>
